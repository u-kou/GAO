...
public class Face {
	...
	public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterFailed onRegisterFailed,
		...
		this.RegisteredPrefix = prefix.toString();
		/*
		 * if(iFace == null){ iFace = new Interface(); }
		 * this.iFace.addRegisteredPrefix(prefix.toString());
		 */
		...
		if (iFace.isConnected(remoteURI) || !(iFace.isRMICN())) {
			...
			try {
				if (message.getType() == MessageType.INTEREST) {
					...
					Interest interest = new Interest(message.getName());
					
					if(interest.getNDNName().contains("RMICN")) 
						interest.setInterestLifetimeMilliseconds(1000* 100);
					this.expressInterest(interest, listener, listener);
					...
					Name prefix;
					if (message.getName().contains("RMICN")) // Data:/RMICN/GWXService
						prefix = new Name("/RMICN/FR0/" + message.getName().substring(7));
					else
						prefix = new Name(message.getName());

					FRCFaceListener listener = new FRCFaceListener(this, prefix);
	...
	public void receive(Packet packet) {
		NDNMessage message = (NDNMessage) packet.getMessage();

		// Log
		// forwarding.getNFD().log("Face", "Receive " +
		// MessageType.toStr(message.getType()),
		// "source: " + packet.getSourceURI() + " dest: " +
		// packet.getDestinationURI()
		// + " name: " + message.getName());
		...
		if (message.getType() == MessageType.INTEREST) {
			...
			// dft.findByLM(message.getName());
			// if (dftEntry != null)
			// dftEntry.getDataFilter().onData(this, new Data(message));
			// else
	...
	public String getRegisteredPrefix() {
	 ...
	 */
	/*
	 * private static ByteBuffer toBuffer(int[] array) { ByteBuffer result =
	 * ByteBuffer.allocate(array.length); for (int i = 0; i < array.length; ++i)
	...
	protected String RegisteredPrefix;
	...
	public class SendInterestListener implements OnData, OnTimeout {
		...
		public void onData(Interest interest, Data data) {
			...
			if (data.getContent().size() == 0) {
				message = new NDNMessage(data.getName().toUri(), MessageType.DATA, null);
			}

			else
				message = new NDNMessage(data.getName().toUri(), MessageType.DATA, data.getContent().toString());

			// if(message.getName().contains("RMICN")){//Data: /RMICN/GWXService
			// message.setName("/RMICN/FR0/" + message.getName().substring(7));
			// }
