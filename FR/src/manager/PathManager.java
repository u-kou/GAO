...
public class PathManager extends MyManager implements ArrivedListener {
	...
	class DCP extends Thread implements Runnable {
		...
		public void run() {
			...
			// if(nfd.getnumCrawl() > 1) {
			// try {
			// Thread.sleep(ParamConfiguration.waitTime * 1000);
			// } catch (InterruptedException e1) {
			// e1.printStackTrace();
			// }
			// }

			System.out.println("Run DCP thread.");

			File file = new File(ParamConfiguration.PathFileAddress);
			try {
				FileWriter filewriter = new FileWriter(file);

				// format: {xLocation}/{yLocation}/ParamConfiguration.altitude
				for (int iGW = 0; iGW < nextGWNames.size(); iGW++) {
					Location GWLocation = ((FlyingRouterNFD) nfd).getInfoManager()
							.getNodeLocation(nextGWNames.get(iGW));
					filewriter.write("/" + (int) GWLocation.x + "/" + (int) GWLocation.y + "/"
							+ ParamConfiguration.altitude + "\n");
				}
			...
			String line = null;
			try {
				ProcessBuilder pb = new ProcessBuilder(ParamConfiguration.DCPCommand);
