...
public class FaceManager extends Manager {
	...
	public Face findOrCreateFace(String remoteURI) {
		...
		if(remoteURI.contains("10.0.0")){
			FRCFaceListener FRControlListener = new FRCFaceListener(
					face, new Name("/Neighbor/" + this.getNfd().getNode().getName() + 
							"/RetrieveRoutes/GW" + remoteURI.substring(remoteURI.length()-1) ));
			try {
				face.registerPrefix(new Name("/Neighbor/" + this.getNfd().getNode().getName() + 
						"/RetrieveRoutes/GW" + remoteURI.substring(remoteURI.length()-1) ), 
			...
			this.nfd.getForwarding().getFaces().put(face.getFaceID(), face);
		}
