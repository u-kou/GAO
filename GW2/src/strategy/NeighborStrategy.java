...
public class NeighborStrategy extends Strategy {
	...
	public void afterReceiveInterest(Face inFace, Interest interest, PITEntry pitEntry) {
		...
		if (nameComponents[2].equals("RegisterFR")) {
			// FR の登録要求に対する処理
			if(nameComponents[3].equals(this.forwarding.getNFD().getNode().getName())){
				System.out.println("Received a RegisterFR Interest whose FR's name is equal to this node.");
				return;
			}
			...
			content = retrieveInfo(inFace, interest);
	...
	private String retrieveRoutes(Face inFace, Interest interest) {
		...
		String retrieveRouteName_toFR = "/Neighbor/FR0/RetrieveRoutes" + 
		"/" + this.forwarding.getNFD().getNode().getName();
		Interest SendInterest = new Interest(retrieveRouteName_toFR, false);
	...
	private String retrieveInfo(Face inFace, Interest interest) {
		// FR・DRで呼ばれる
