...
public class InfoManager extends MyManager implements L2ConnectedListener, FaceConnectedListener {
	...
	private void updateFIB(Face face, List<String> names) {
    	for (String name: names) {
    		...
    		if (fibEntry == null) {
    			ArrayList<Face> Nexthops = new ArrayList<Face>();
    			Nexthops.add(face);
    			if(name.contains("RMICN")) 
    				fibEntry = new FIBEntry("/RMICN/FR0/" + name.substring(7, 10) + "Service", Nexthops);
    			else fibEntry = new FIBEntry(name, Nexthops);
