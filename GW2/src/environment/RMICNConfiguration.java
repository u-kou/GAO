...
public class RMICNConfiguration {
	...
	public Location caluculateDepot() {
		/*
		 * int gwNum = 0; float x = 0; float y = 0; for (Node temp:
		 * env.getNodes().values()) { RMICNNode node = (RMICNNode) temp; if
		 * (node.getNodeType() != RMICNNode.TYPE_GW) continue; x +=
		 * node.getNowLocation().x; y += node.getNowLocation().y; gwNum++; }
		 * 
	...
	public void OneFRNetworkPattern() {
		...
		int MaxInterestSize = 3;
		int NowInterestSize = 0;
		boolean ret = false;
		while (NowInterestSize < MaxInterestSize) {
			RMICNApplication a1 = applications.get(0);
			RMICNApplication a2 = applications.get(1);
			//ret = a2.sendInterest(a1.getInterestFilterName() + "/" + "FR0" + "/" + System.currentTimeMillis());
			ret = a2.sendInterest("/RMICN" + "/" + "FR0" + "/" + "GW1Service") ;
			if (ret) NowInterestSize++;
			try {
				Thread.sleep(200);
			} catch (InterruptedException e) {
				e.printStackTrace();
