...
//		scanner.close();
		...
		// DRPathManager pathManager = ((DRPathManager)
		// dr.getNFD().getPathManager());
		// pathManager.deliver();

		/*
		 * if (ParamConfiguration.network == 0) // センサネットワーク
		 * nc.sensorNetworkPattern(ParamConfiguration.packetNum); else if
		 * (ParamConfiguration.network == 1) // 災害ネットワーク
		 * nc.disasterNetworkPattern(ParamConfiguration.packetNum);
		 */

		nc.OneFRNetworkPattern();
