...
public class Face {
		...
		if (iFace.isConnected(remoteURI) || !(iFace.isRMICN())) {
			...
			try {
				if (message.getType() == MessageType.INTEREST) {
					...
					// set InterestLifeTime for Interest:/RMICN/GWXService
	...
	public void receive(Packet packet) {
		...
		if (message.getType() == MessageType.INTEREST) {
			InterestFilterTableEntry iftEntry = (InterestFilterTableEntry) ift.findByLM(message.getName());
			if (iftEntry != null)
				iftEntry.getInterestFilter().onInterest(this, new Interest(message));
			else
				forwarding.onReceiveInterest(this, new Interest(message));
		} else if (message.getType() == MessageType.DATA) {
			DataFilterTableEntry dftEntry = (DataFilterTableEntry) dft.findByLM(message.getName());
			if (dftEntry != null)
