...
public class FlyingRouter extends RMICNNode {
	...
	public FlyingRouter(String name, String interfaceURI, RMICNEnvironment environment
		...
		if (isDepotRouter) {
			...
			Location GWLocation = this.getEnvironment().getNode("GW1").getNowLocation();
			this.setNowLocation(GWLocation.x, GWLocation.y);
	/*		
			...
			FRCFaceListener NeighborListener = new FRCFaceListener(
					NeighborFace, new Name("/Neighbor"));
			...
			try {
				...
				NeighborFace.registerPrefix(new Name("/Neighbor"), 
