...
public class InfoManager extends MyManager implements L2ConnectedListener, FaceConnectedListener {
	...
	public void onFaceDisconnected(Face face) {
		if(this.getNFD().getNode().getName() == "DR"){
			FlyingRouterNFD drNFD =  (FlyingRouterNFD)this.nfd;
			if(drNFD.getStatusRetrieveInfo() != RMICNFD.NOTSendInterest){
				drNFD.CrawlRemainTime = ((DRPathManager)drNFD.getPathManager()).getworstCrawlingTime();
			}
			drNFD.setStatusRetrieveInfo(RMICNFD.NOTSendInterest);
			drNFD.CrawlRemainTime -= 200;
			if(drNFD.CrawlRemainTime <= 0){
				FlyingRouter frNode = (FlyingRouter)drNFD.getNode().getEnvironment().getNode("FR0");
				frNode.setNowLocation(0, 0);
			}
		}
