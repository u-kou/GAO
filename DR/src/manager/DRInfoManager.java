...
public class DRInfoManager extends FRInfoManager {
	...
	public void onFaceConnected(Face face) {
		...
		if (nte != null && nte.getNodeType() == RMICNNode.TYPE_FR &&
				((RMICNFD)face.getForwarding().getNFD()).getStatusRetrieveInfo() == RMICNFD.NOTSendInterest) {
			...
			((RMICNFD)face.getForwarding().getNFD()).setStatusRetrieveInfo(RMICNFD.NOTReceiveData);
			*/
		}
	}
	...
	public void updateInfo(Face face, Info info) {
		...
		if (pathManager.isAllFRsAtDepot()) {
			...
			System.out.println("DRPathManager: retrieved information from all FRs.");
			onDRReady();
		}
	}
	...
	public void onDRReady() {
		...
		for (Entry<Integer, FaceTableEntry> e: ft.getTable().entrySet()) {
			...
			String NodeName = e.getValue().getNodeName();
			String onDRReadyName = "/Neighbor/" + NodeName + "/onDRReady";
