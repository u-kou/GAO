...
public class Interest implements ChangeCountable {
  ...
  public
  Interest(Name name, boolean t_cacheflag)
  {
    if (name != null){
      name_.set(new Name(name));
    	this.name = name.toUri();
    }
    cacheFlag = t_cacheflag;
  }
  ...
  Interest(Interest interest)
  {
    ...
    this.name = interest.getName().toUri();
  ...
  //added
	...
	public Interest(String name) {
		NDNMessage(name, MessageType.INTEREST);
	}
	
	public Interest(String name, boolean cacheFlag) {
		NDNMessage(name, MessageType.INTEREST, cacheFlag);
	}
	
	public Interest(NDNMessage message){
		NDNMessage(message.getName(), message.getType(), true);
	}
	
	public void NDNMessage(String name, int type) {
		NDNMessage(name, type, true);
	}
	
	public void NDNMessage(String name, int type, boolean cacheFlag) {
		this.name = NameUtility.filter(name);
		this.type = type;
		this.cacheFlag = cacheFlag;
		this.requestedTime = 0;
		
	    if (name != null) name_.set(new Name(name));
